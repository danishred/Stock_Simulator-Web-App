...
def buy():
    ...
    if request.method == "POST":
        ...
        # Debiting cash
        obj.update_user_cash(
            usrcsh - Decimal(shares) * Decimal(buystks["price"]), session["user_id"]
        )
...
def register():
    ...
    if request.method == "POST":
        ...
        if not re.match(pattern, password):
            return apology(
                "The password should contain at least one lowercase letter, one uppercase letter, one digit, and one special character"
            )
...
def sell():
    ...
    if request.method == "POST":
        ...
        obj.update_liveindex_shares(
            int(sharescnt) - int(shares), session["user_id"], symbol.upper()
        )
...
def notes():
    ...
    if request.method == "GET":
        ...
        return render_template(
            "notes.html", notesdicts=notesdicts, currentuser=currentuser
        )
